...
export function MovimentosPage() {
    ...
    const filtroMovimentacoes = movimentacoes?.filter(movimento =>
        ...
        movimento.descricao.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').includes(pesquisa.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')) ||
        movimento.tipo.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').includes(pesquisa.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')) ||
        movimento.pagamento.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').includes(pesquisa.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')) ||
        movimento.valor.toString().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').includes(pesquisa.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, ''))
    ) || [];
